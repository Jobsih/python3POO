...
class  AyudanteProfesor(Alumno,Profesor):
    ...
    def __str__(self):
        return Alumno.__str__(self) + f" Numero detrabajador: {self.numero_trabajador} Area: {self.area} Horas de trabajo: {self.horas}"

    def dar_clase(self, materia):
        print(f"{self.nombre} esta dando clases de {materia} del área {self.area} por {self.horas} horas")
